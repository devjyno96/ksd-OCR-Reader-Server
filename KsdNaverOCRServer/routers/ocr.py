...
from KsdNaverOCRServer.schemas.ocr import RequestOCR, RequestOCRByUser
...
# def ocr_request_v2_by_file(user_id: int, category: CategoryEnum = Form(...), image_file: UploadFile = File(...),
...
def ocr_request_v2_by_url(request: RequestOCRByUser, db: Session = Depends(get_db)):
    '''
    ...
    if request.category == CategoryEnum.Total:
        ...
        return ocr_repository.ocr_request_v2_by_url(user_id=request.user_id, image_url=request.image_url,
                                                    file_name_extension=request.file_name_extension,
                                                    category=request.category, db=db)
